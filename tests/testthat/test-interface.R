...
test_that("peak_exists is FALSE when no peak in range of given coordinates", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    expect_false(this_db$peak_exists(-74.080, 42.162))
  }
})

test_that("add_peak only requires lat and lon data", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    expect_no_condition(this_db$add_peak(-74.080, 42.162))
  }
})

test_that("add_peak works, and then peak_exists finds it", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    ...
    peak_elev_m <- 1114
    peak_elev_ft <- 3655
    peak_id <- expect_no_error(this_db$add_peak(peak_lat, peak_lon, peak_name,
                                                peak_elev_m, peak_elev_ft))
    expect_true(this_db$peak_exists(round(peak_lat, digits=3), round(peak_lon, digits=3)))
  }
})

test_that("add_peak, when given one of elev_m or elev_ft, calculates the other", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    peak_name <- peaks$peak_id[1]
    peak_coords <- sf::st_coordinates(peaks$geometry[1])
    ...
    # peak_elev_m <- 1114
    peak_elev_ft <- 3655
    new_peak_id <- expect_no_error(this_db$add_peak(peak_lat, peak_lon,
                                                name=peak_name,
                                                elev_ft=peak_elev_ft))
    this_peak <- this_db$tbl("peaks") |>
      dplyr::filter(peak_id == new_peak_id) |>
      dplyr::collect()
    expect_equal(nrow(this_peak), 1)
    expect_equal(this_peak$peak_elev_ft, 3655)
    expect_equal(this_peak$peak_elev_m, 1114)
...
test_that("add_peak_sfdf works", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    peak_ids <- expect_no_error(this_db$add_peak_sfdf(peaks))
    expect_true(all(!is.na(peak_ids)))
    expect_equal(nrow(peaks), length(peak_ids))
  }
})

test_that("add_peak_sfdf line failure rolls back full transaction", {
  for (db in supported_databases()) {
    this_db <- expect_no_condition(make_testdbobj(db, create_tables=TRUE))
    bad_peaks <- peaks
    bad_peaks$geometry[3] <- NA
    peak_ids <- expect_error(this_db$add_peak_sfdf(bad_peaks))
    db_table_rows <- this_db$tbl("peaks") |> dplyr::count() |> dplyr::pull("n")
